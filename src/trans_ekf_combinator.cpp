...
namespace riptide_gnc
{
   TransEKFCombinator::TransEKFCombinator(ros::NodeHandle nh)
   {
      ...
      */
      ...
      odometrySub = nh_.subscribe<nav_msgs::Odometry>("/puddles/odometry/filtered", 1, &TransEKFCombinator::odometryCB, this);
      six_dof_pub_ = nh_.advertise<auv_msgs::SixDoF>("/puddles/auv_gnc/trans_ekf/six_dof1", 1);
   ...
   */
   ...
   void TransEKFCombinator::odometryCB(const nav_msgs::Odometry::ConstPtr &odom){
      double time1 = six_dof_msg_.header.stamp.toSec(); // Current stamp
      double time2 = odom->header.stamp.toSec();         // new stamp
      if (time2 > time1)
         six_dof_msg_.header.stamp = odom->header.stamp;

      //ROS_INFO("MADE IT TO ODOM CB");
      six_dof_msg_.pose = odom->pose.pose;
      six_dof_msg_.velocity = odom->twist.twist;
      cb_counter_++;
      //six_dof_msg.linear_accel = odom.linear_accel;
   }
